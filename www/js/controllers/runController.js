...
//TODO: CLEAR VALUES AFTER RUN SUMMARY, NEW RUN BUTTON?
  ...
  .controller('RunCtrl', function($scope, $cordovaInsomnia, $window, $rootScope,$ionicPlatform, $cordovaGeolocation,  $ionicLoading,$ionicPopup, $interval, AppAPI, UserAPI, RunAPI, CharityAPI, HistoryAPI, DonationAPI, $timeout, $ionicModal, $ionicTabsDelegate){
    $scope.isDetailDisplayed = false;
    $scope.isRunDetailDisplayed = false;
    $scope.isReadyToRun = false;
    $scope.isPaused = false;

    console.log('SELECTED TAB: ' + $ionicTabsDelegate.selectedIndex());

    console.log('isReadyToRun ' + $scope.isReadyToRun);
    console.log('isRunning ' + $scope.isRunning);
    console.log('isPaused ' + $scope.isPaused);
    ...
    $rootScope.$on('initRun', function() {
      ...
      $scope.startRunBtnTpd = function () {
        var test = localStorage.getItem('readyToRun');
        console.log(test);
        if($scope.isReadyToRun) {
      ...
      $scope.resumeBtnTpd = function () {
        $scope.resumeRun();
        $scope.togglePause();
      }

      $scope.stopButtonTpd = function () {
        $scope.showEnsureStop();
      }

      $scope.displayThisBtnRun = function () {
        if ($scope.isRunning && !$scope.isPaused) {
          return true;
        } else {
          return false;
        }
      ...
      $scope.displayThisBtnPaused = function () {
        if ($scope.isRunning && $scope.isPaused) {
          return true;
        } else {
          return false;
...
/*
      ...
      $scope.locateControl = function(locateDiv, map){
        ...
        $scope.addOriginalPositionLocateUI = function(){
          locateUI.style.position = 'absolute';
          locateUI.style.bottom = '80px';
          locateUI.style.left = '80vw';
          locateDiv.appendChild(locateUI);
        }
        ...
        $scope.mapCreated = function(map){
          ...
          $scope.onSuccess = function(pos){
            ...
            $scope.centerOnMe = function(){
              ...
              console.log('%cDone Centering', 'color: Green');
            };
            $rootScope.hide();
            $scope.$broadcast('scroll.refreshComplete');
          }
...
/* Commenting out the start control
...
*/
      ...
      $scope.stopRun = function(){
        ...
        $scope.postRun();
        $scope.stopTimer();
        $window.location.href = ('#/app/historyDay'); //MOVED THE ROUTE CHANGE HERE FROM success of RunAPI.saveRun-----------------
        $scope.resetRun();
      ...
      $scope.postRun = function () {
        ...
        RunAPI.saveRun(form)
          .success(function(data, status, headers, config){
            ...
            /*Moving this up in the flow to see if we can git rid of the delay between stopping te run and going to history
